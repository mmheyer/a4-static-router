...
std::optional<RoutingEntry> RoutingTable::getRoutingEntry(ip_addr ip) {
    ...
    uint32_t ip_network_order = ntohl(ip);

    for (const auto& entry : routingEntries) {
        uint32_t dest_ip_host_order = ntohl(entry.dest);
        uint32_t mask_host_order = ntohl(entry.mask);

        uint32_t network = dest_ip_host_order & mask_host_order;
        uint32_t ip_network = ip_network_order & mask_host_order;
std::cout << "Dest IP: " << inet_ntoa(*(in_addr*)&dest_ip_host_order) 
                  << " | Mask: " << inet_ntoa(*(in_addr*)&mask_host_order) 
                  << " | Network: " << inet_ntoa(*(in_addr*)&network) 
                  << " | IP: " << inet_ntoa(*(in_addr*)&ip_network_order) 
                  << std::endl;
